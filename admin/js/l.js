    ...
    $(document).ready(function () {
        ...
        function delete_task(el) {
            ...
            if (id != undefined && id > 0) {
                if (confirm("Are you sure? Agents data will also be deleted")) {
                    $.post('/admin/task.php', {'action': 'delete', 'id': id}, function (data) {
                        if (data == 'OK') {
        ...
        $('#button-task-create').click(function () {
            ...
            var type = $('#input_task_type').val();
            var group = $('#input_task_group').val();
            var additional = $('#input_task_additional').val();
            if (name == null || type == null || group == null) {
                alert('Fill fields');
            } else {
                var data = {'action': 'add', 'id': group, 'name': name, 'type': type, 'additional': additional};
                $.post('/admin/task.php', data, function (json) {
                    try {
                        data = JSON.parse(json);
                        if (data.status == "success") {
                            var group_name = $('#input_task_group option:selected').text();
                            var type_name = $('#input_task_type option:selected').text();
